...
from database import get_db_connection, initialize_db, execute_query, fetch_one, fetch_all
import os
import time
...
def signin():
    ...
    user = fetch_one("SELECT user_id, name, email, password_hash FROM Users WHERE email = ?", (email,))
...
def get_user(user_id):
    ...
    user = fetch_one("SELECT user_id, name, email, created_at FROM Users WHERE user_id = ?", (user_id,))
    if user:
...
def update_user(user_id):
    ...
    if email:
        existing_user_with_email = fetch_one("SELECT user_id FROM Users WHERE email = ? AND user_id != ?", (email, user_id))
...
def delete_user(user_id):
    if g.user_id != user_id:
        return jsonify({"error": "Access denied. You can only delete your own profile."}), 403

    data = request.get_json()
    password_confirmation = data.get('password_confirmation')

    if not password_confirmation:
        return jsonify({"error": "Password confirmation is required to delete account."}), 400

    user = fetch_one("SELECT password_hash FROM Users WHERE user_id = ?", (user_id,))
    if not user or not check_password_hash(user['password_hash'], password_confirmation):
...
def get_courses():
    ...
    if courses:
        ...
        return jsonify({"message": "No courses found for this user (as host or attendee). Please create or join one."}), 404

@app.route('/courses/<int:course_id>', methods=['GET'])
@login_required
def get_course(course_id):
    course = fetch_one("SELECT C.*, U.name AS host_name FROM Courses C JOIN Users U ON C.host_id = U.user_id WHERE course_id = ?", (course_id,))
    if not course:
        return jsonify({"error": "Course not found."}), 404

    is_host = (course['host_id'] == g.user_id)
    is_enrolled = fetch_one("SELECT 1 FROM Enrollments WHERE user_id = ? AND course_id = ?", (g.user_id, course_id))

    if not is_host and not is_enrolled:
        return jsonify({"error": "Access denied. You are not authorized to view this course."}), 403
...
def update_course(course_id):
    ...
    if course['host_id'] != g.user_id:
        return jsonify({"error": "Access denied. You are not the host of this course."}), 403
    ...
    if name:
        ...
        params.append(name)
    if join_code:
        existing_course_with_code = fetch_one("SELECT course_id FROM Courses WHERE join_code = ? AND course_id != ?", (join_code, course_id))
        if existing_course_with_code:
            return jsonify({"error": "Join code already in use by another course."}), 409
...
def enroll_in_course():
    ...
    course = fetch_one("SELECT course_id, name, host_id FROM Courses WHERE join_code = ?", (join_code,))
    if not course:
        return jsonify({"error": "Course not found with this join code."}), 404

    if course['host_id'] == g.user_id:
        return jsonify({"error": "You are the host of this course; you cannot enroll as an attendee."}), 400
...
def get_user_enrollments(user_id):
    if g.user_id != user_id:
        return jsonify({"error": "Access denied. You can only view your own enrollments."}), 403
...
def get_course_attendees(course_id):
    course = fetch_one("SELECT host_id, name FROM Courses WHERE course_id = ?", (course_id,))
    if not course:
        return jsonify({"error": "Course not found."}), 404
    if course['host_id'] != g.user_id:
        return jsonify({"error": "Access denied. You are not the host of this course."}), 403
...
def unenroll_from_course():
    ...
    enrollment = fetch_one("SELECT * FROM Enrollments WHERE user_id = ? AND course_id = ?", (g.user_id, course_id))
    if not enrollment:
        return jsonify({"error": "You are not enrolled in this course."}), 404

    try:
        execute_query("DELETE FROM Enrollments WHERE user_id = ? AND course_id = ?", (g.user_id, course_id))
...
def create_session(course_id):
    ...
    try:
        ...
        return jsonify({"error": "Invalid start_time or end_time format. Use Unix epoch integers."}), 400

    try:
        query = "INSERT INTO Sessions (course_id, start_time, end_time) VALUES (?, ?, ?)"
...
def get_course_sessions(course_id):
    ...
    if not is_host and not is_enrolled:
        return jsonify({"error": "Access denied. You are not authorized to view sessions for this course."}), 403

    sessions = fetch_all("SELECT * FROM Sessions WHERE course_id = ? ORDER BY start_time DESC", (course_id,))
    if sessions:
        return jsonify([dict(s) for s in sessions]), 200
    else:
        return jsonify({"message": "No sessions found for this course."}), 404

@app.route('/sessions/<int:session_id>', methods=['GET'])
@login_required
def get_session(session_id):
    session_data = fetch_one("SELECT * FROM Sessions WHERE session_id = ?", (session_id,))
    if not session_data:
    ...
    if not is_host and not is_enrolled:
        return jsonify({"error": "Access denied. You are not authorized to view this session."}), 403
...
def update_session(session_id):
    session_data = fetch_one("SELECT course_id, start_time, end_time FROM Sessions WHERE session_id = ?", (session_id,))
    if not session_data:
        return jsonify({"error": "Session not found."}), 404
    ...
    if not course or course['host_id'] != g.user_id:
        return jsonify({"error": "Access denied. You are not the host of this course or course not found."}), 403
    ...
    if start_time is not None:
        try:
            ...
            return jsonify({"error": "Invalid start_time format. Use Unix epoch integer."}), 400
    ...
    if end_time is not None:
        try:
            ...
            return jsonify({"error": "Invalid end_time format. Use Unix epoch integer."}), 400
    ...
    if proposed_end_time is not None and proposed_start_time is not None and proposed_end_time < proposed_start_time:
        return jsonify({"error": "End time cannot be before start time."}), 400
...
def delete_session(session_id):
    session_data = fetch_one("SELECT course_id FROM Sessions WHERE session_id = ?", (session_id,))
    if not session_data:
        return jsonify({"error": "Session not found."}), 404
...
def mark_attendance(session_id):
    ...
    course_id = session_data['course_id']
    enrollment = fetch_one("SELECT 1 FROM Enrollments WHERE user_id = ? AND course_id = ?", (attendee_id, course_id))
    if not enrollment:
        return jsonify({"error": "You are not enrolled in this course."}), 403

    # Check if user is the host
    course = fetch_one("SELECT host_id, geolocation_latitude, geolocation_longitude, late_threshold_minutes, present_threshold_minutes FROM Courses WHERE course_id = ?", (course_id,))
    if course['host_id'] == attendee_id:
        return jsonify({"error": "Hosts cannot mark attendance for their own sessions."}), 400
    ...
    session_end_time = session_data['end_time'] if session_data['end_time'] else (session_start_time + 3600) # Default to 1 hour if no end_time

    late_threshold_seconds = course['late_threshold_minutes'] * 60
    present_threshold_seconds = course['present_threshold_minutes'] * 60

    if session_start_time - present_threshold_seconds <= current_time <= session_end_time + late_threshold_seconds:
        is_within_time = True
    ...
    if not is_within_time:
        return jsonify({"error": "Attendance cannot be marked. Session is not active or has ended."}), 400

    # Geolocation check
    if course['geolocation_latitude'] is not None and course['geolocation_longitude'] is not None:
        ...
        try:
            ...
            if distance > 0.1: # Example: 100 meters radius
                return jsonify({"error": "You are not within the required proximity to mark attendance."}), 400
        except (ValueError, TypeError):
    ...
    existing_attendance = fetch_one("SELECT * FROM Attendances WHERE user_id = ? AND session_id = ?", (attendee_id, session_id))
    ...
    try:
        ...
        attendance_id = execute_query(
            query,
            (attendee_id, session_id, status, current_time,
             user_latitude, user_longitude, proof_base64)
        )
        return jsonify({
...
def mark_absent_for_unattended(session_id):
    ...
    course_data = fetch_one("SELECT host_id, late_threshold_minutes, present_threshold_minutes FROM Courses WHERE course_id = ?", (course_id,))
    if not course_data:
        return jsonify({"error": "Course not found."}), 404 # Should not happen if session_data was found

    if course_data['host_id'] != user_id:
        return jsonify({"error": "Forbidden: Only the course host can mark unattended attendees absent."}), 403
    ...
    late_threshold_seconds = course_data['late_threshold_minutes'] * 60 if course_data['late_threshold_minutes'] is not None else 0

    if current_time <= (session_end_time + late_threshold_seconds):
        return jsonify({"error": "Cannot mark absent: Session attendance window is still open or has not started."}), 400
    ...
    try:
        # 4. Identify enrolled attendees who do NOT have an attendance record for this session
        ...
        """
        cursor.execute(unmarked_attendees_query, (course_id, session_id))
        unmarked_attendees = cursor.fetchall()

        if not unmarked_attendees:
            return jsonify({"message": "No unattended attendees found to mark absent for this session."}), 200

        # 5. Insert 'Absent' records for each identified user
        for attendee in unmarked_attendees:
            ...
            try:
                ...
                errors.append(f"Failed to mark {attendee['name']} ({attendee['email']}) absent: {e}")
            except RuntimeError as e:
                errors.append(f"Database error for {attendee['name']} ({attendee['email']}): {e}")

        conn.commit() # Commit changes after loop

        response_message = f"Successfully marked {marked_absent_count} unattended attendees as Absent."
        if errors:
            response_message += " Some errors occurred: " + "; ".join(errors)

        return jsonify({"message": response_message, "marked_absent_count": marked_absent_count, "errors": errors}), 200

    except Exception as e:
        conn.rollback() # Rollback on any major error
...
def get_attendance(attendance_id):
    attendance = fetch_one("SELECT * FROM Attendances WHERE attendance_id = ?", (attendance_id,))
    if not attendance:
        return jsonify({"error": "Attendance record not found."}), 404

    # Get associated session and course to check authorization
    session_data = fetch_one("SELECT course_id FROM Sessions WHERE session_id = ?", (attendance['session_id'],))
    if not session_data:
        return jsonify({"error": "Associated session not found."}), 500
    course = fetch_one("SELECT host_id FROM Courses WHERE course_id = ?", (session_data['course_id'],))
    if not course:
        return jsonify({"error": "Associated course not found."}), 500
    ...
    if attendance['user_id'] != g.user_id and course['host_id'] != g.user_id:
        return jsonify({"error": "Access denied. You are not authorized to view this attendance record."}), 403
...
def get_user_all_attendances(user_id):
    if g.user_id != user_id:
        return jsonify({"error": "Access denied. You can only view your own attendance records."}), 403
...
def get_course_attendance_summary(course_id):
    ...
    if not is_host and not is_enrolled:
        return jsonify({"error": "Access denied. You are not authorized to view this course's attendance summary."}), 403
    ...
    if is_host:
        ...
        if summary_data:
            return jsonify({
                "course_id": course_id,
                "course_name": course['name'],
                "summary_for_all_attendees": [dict(row) for row in summary_data]
            }), 200
        else:
            return jsonify({"message": "No attendance summary available for this course yet."}), 404
    # For attendee: get their own summary
        ...
        if summary_data:
            ...
            return jsonify({"message": "No attendance summary available for you in this course yet."}), 404
